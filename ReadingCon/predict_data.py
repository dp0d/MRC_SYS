...
def predict_data(question_text, doc_tokens, tokenizer, max_seq_length, max_query_length):
    ...
    return (torch.LongTensor(input_ids).unsqueeze(0), torch.LongTensor(input_ids_q).unsqueeze(0),
            torch.LongTensor(input_mask).unsqueeze(0), torch.LongTensor(input_mask_q).unsqueeze(0),
